...
constexpr std::array<AutonRoutine, AUTON_OPTION_COUNT> AUTON_OPTIONS = {
    AutonRoutine::DoNothing, AutonRoutine::Left, AutonRoutine::Right};

constexpr double WHEEL_DIAMETER_IN = 2.25;
constexpr double DRIVE_GEAR_RATIO = 1.0;
constexpr double WHEEL_CIRCUMFERENCE_M =
    WHEEL_DIAMETER_IN * 0.0254 * 3.14159265358979323846;
...
enum class ImuOverlayState { Hidden, Calibrating, Success, Failed };
...
MotorRunSummary summarizeMotorVoltages(
    const std::vector<std::int32_t>& voltages) {
    int runningCount = 0;
    int positiveCount = 0;
    int negativeCount = 0;
...
double driveSpeedMps() { return driveSpeedRpm() * RPM_TO_MPS; }

double leftDriveAvgTempC() { return average(leftDrive.get_temperature_all()); }
...
double intakeAvgTempC() {
    return average(
        {leftIntake.get_temperature(), rightIntake.get_temperature()});
}
...
void stylePillButton(lv_obj_t* button, bool active) {
    ...
    lv_obj_set_style_bg_color(
        button, active ? lv_color_hex(0x3B84FF) : lv_color_hex(0x2A3A4D), 0);
...
lv_obj_t* createPillButton(lv_obj_t* parent, const char* text, int x, int y,
                           int w, int h) {
...
void setTopStatus() {
    ...
    lv_label_set_text_fmt(ui.statusLabel, "Aut: %s",
                          autonRoutineName(selectedAuton.load()));
}
...
void setTempDot(double maxTemp) {
    ...
    lv_obj_set_style_bg_color(ui.tempDot,
                              maxTemp >= MOTOR_TEMP_WARN_C
                                  ? lv_color_hex(0xE5484D)
                                  : lv_color_hex(0x29C56A),
                              0);
}
...
void setScaledParts100(double value, int& whole, int& frac, bool& negative) {
    splitFixed100(static_cast<int>(std::lround(value * 100.0)), whole, frac,
                  negative);
}
...
void refreshAutonSelector() {
    ...
    lv_label_set_text_fmt(ui.autonSelectedLabel, "Act: %s",
                          autonRoutineName(active));
}

void refreshProfileSelector() {
    const DriverProfile active = selectedProfile.load();
    styleProfileButton(ui.profileWhiteButton, DriverProfile::White,
                       active == DriverProfile::White);
    styleProfileButton(ui.profilePinkButton, DriverProfile::Pink,
                       active == DriverProfile::Pink);
}
...
void onShowImagePressed(lv_event_t*) { showImageRequested.store(true); }

void onHideImagePressed(lv_event_t*) { showImageRequested.store(false); }

lv_obj_t* createSectionCard(lv_obj_t* screen, int x, int y, int w, int h,
                            lv_color_t color) {
...
void buildTopBar(lv_obj_t* screen) {
    ...
    if (ui.profileWhiteButton != nullptr) {
        lv_obj_set_style_text_font(ui.profileWhiteButton,
                                   &lv_font_montserrat_10, 0);
        lv_obj_add_event_cb(ui.profileWhiteButton, onWhiteProfilePressed,
                            LV_EVENT_CLICKED, nullptr);
    }

    ui.profilePinkButton = createPillButton(topBar, "PINK", 332, 4, 42, 22);
    if (ui.profilePinkButton != nullptr) {
        lv_obj_set_style_text_font(ui.profilePinkButton, &lv_font_montserrat_10,
                                   0);
        lv_obj_add_event_cb(ui.profilePinkButton, onPinkProfilePressed,
                            LV_EVENT_CLICKED, nullptr);
    }
    ...
    if (ui.showImageButton != nullptr) {
        ...
        lv_obj_set_style_text_font(ui.showImageButton, &lv_font_montserrat_10,
                                   0);
        lv_obj_add_event_cb(ui.showImageButton, onShowImagePressed,
                            LV_EVENT_CLICKED, nullptr);
    }
...
void buildSections(lv_obj_t* screen) {
    ui.infoCard = createSectionCard(screen, INFO_X, INFO_Y, INFO_W, INFO_H,
                                    lv_color_hex(0x1B2837));
    ...
    ui.autonCard = createSectionCard(screen, AUTON_X, AUTON_Y, AUTON_W, AUTON_H,
                                     lv_color_hex(0x1B2837));
    ui.autonSelectedLabel = lv_label_create(ui.autonCard);
    lv_obj_set_pos(ui.autonSelectedLabel, 4, 4);
    lv_obj_set_style_text_font(ui.autonSelectedLabel, &lv_font_montserrat_14,
                               0);
    ...
    for (int i = 0; i < AUTON_OPTION_COUNT; i++) {
        ui.autonButtons[i] =
            createPillButton(ui.autonList, autonRoutineName(AUTON_OPTIONS[i]),
                             2, 2 + (i * 36), AUTON_W - 36, 30);

        lv_obj_add_event_cb(
            ui.autonButtons[i], onAutonOptionPressed, LV_EVENT_CLICKED,
...
void buildImageScreen(lv_obj_t* screen) {
    ...
    lv_label_set_text(ui.imageHintLabel,
                      "Drive: Curvature\nTap image to return");
    ...
    if (ui.imageObj != nullptr) {
        ...
        lv_obj_add_event_cb(ui.imageObj, onHideImagePressed, LV_EVENT_CLICKED,
                            nullptr);
    } else {
...
void buildImuOverlay(lv_obj_t* screen) {
    ...
    lv_label_set_text(ui.imuOverlayLabel,
                      "IMU Calibrating...\nKeep robot still");
    lv_obj_align(ui.imuOverlayLabel, LV_ALIGN_CENTER, 0, -24);

    ui.imuOverlayExitButton =
        createPillButton(ui.imuOverlay, "Exit", 192, 190, 96, 30);
...
void startFlashSequence(bool enabled) {
    ...
    flashState.color =
        enabled ? lv_color_hex(0x1E8F3E) : lv_color_hex(0xB31212);
...
void applyFlashOverlay() {
    ...
    const int request =
        pendingFlashRequest.exchange(FLASH_REQ_IDLE, std::memory_order_relaxed);
    ...
    if ((nowMs - flashState.phaseStartMs) >= FLASH_OFF_MS &&
        flashState.pulsesShown < FLASH_PULSE_COUNT) {
        flashState.visible = true;
        flashState.phaseStartMs = nowMs;
        lv_obj_set_style_bg_color(ui.flashOverlay, flashState.color, 0);
        moveObjToFront(ui.flashOverlay);
...
void applyImuOverlay() {
    ...
    const bool calibrating =
        inertial.is_calibrating() || imuStatus == pros::ImuStatus::calibrating;
    const bool failed =
        !inertial.is_installed() || imuStatus == pros::ImuStatus::error;
...
void updateTelemetry(double speedMps) {
    ...
    const double dt = std::max(
        0.001, static_cast<double>(nowMs - telemetry.lastTimeMs) / 1000.0);
...
void updateDriveModeDisplay(const char* driveMode) {
    if (ui.imageHintLabel != nullptr) {
        lv_label_set_text_fmt(ui.imageHintLabel,
...
void updateControllerMotorSummary(DashboardMode mode) {
    ...
    const std::vector<std::int32_t> rightVoltages =
        rightDrive.get_voltage_all();
    drivetrainVoltages.insert(drivetrainVoltages.end(), rightVoltages.begin(),
                              rightVoltages.end());
    const MotorRunSummary drivetrainSummary =
        summarizeMotorVoltages(drivetrainVoltages);

    const std::vector<std::int32_t> intakeVoltages = {rightIntake.get_voltage(),
                                                      leftIntake.get_voltage()};
    const MotorRunSummary intakeSummary =
        summarizeMotorVoltages(intakeVoltages);

    master.print(0, 0, "DV R%02d D%2d", drivetrainSummary.runningCount,
                 drivetrainSummary.direction);
    master.print(1, 0, "IN R%02d D%2d", intakeSummary.runningCount,
                 intakeSummary.direction);
    ...
    // master.print(2, 1, "Intake running: %s",
    // robotactions::isAnyIntakeRunning());
}

void updateInfoSection(int batteryPercent, int batteryMillivolts,
                       double leftDriveTemp, double rightDriveTemp,
                       double intakeTemp, double maxTemp, const char* driveMode,
                       double imuHeadingDeg, double odomXIn, double odomYIn) {
    int voltageWhole = 0;
    int voltageFrac = 0;
    ...
    setScaledParts100(telemetry.accelMps2, accelWhole, accelFrac,
                      accelNegative);

    int headingWhole = 0;
    int headingFrac = 0;
    bool headingNegative = false;
    setScaledParts100(imuHeadingDeg, headingWhole, headingFrac,
                      headingNegative);
    ...
    lv_label_set_text_fmt(ui.infoLabel,
                          "Bat %d%% %d.%02dV\n"
                          "Drv %s\n"
                          "\n"
                          "Spd %s%d.%02d m/s\n"
                          ...
                          "Tmp L/R %d/%dC\n"
                          "Intk %dC Max %dC",
                          batteryPercent, voltageWhole, voltageFrac, driveMode,
                          speedNegative ? "-" : "", speedWhole, speedFrac,
                          accelNegative ? "-" : "", accelWhole, accelFrac,
                          headingNegative ? "-" : "", headingWhole, headingFrac,
                          odomXNegative ? "-" : "", odomXWhole, odomXFrac,
                          odomYNegative ? "-" : "", odomYWhole, odomYFrac,
...
void dashboardTask(void*) {
    ...
    while (true) {
        ...
        const int batteryPercent =
            static_cast<int>(std::lround(pros::battery::get_capacity()));
        ...
        updateInfoSection(batteryPercent, batteryMillivolts, leftDriveTemp,
                          rightDriveTemp, intakeTemp, maxTemp, driveMode,
                          imuHeadingDeg, odomPose.x, odomPose.y);
...
void initDashboard() {
    ...
    static pros::Task uiTask(dashboardTask, nullptr, TASK_PRIORITY_DEFAULT - 1,
                             TASK_STACK_DEPTH_DEFAULT, "Dashboard");
    (void)uiTask;
}

void setDashboardMode(DashboardMode mode) {
    const DashboardMode previousMode = requestedMode.exchange(mode);
    if (mode == DashboardMode::Driver &&
        previousMode != DashboardMode::Driver) {
        showImageRequested.store(true);
    }
}

AutonRoutine getSelectedAutonRoutine() { return selectedAuton.load(); }

const char* getSelectedAutonName() {
    return autonRoutineName(selectedAuton.load());
}

DriverProfile getSelectedDriverProfile() { return selectedProfile.load(); }
